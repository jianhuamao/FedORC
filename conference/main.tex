...
The corresponding CIFAR-10 and MedMNIST results are provided in Appendix~\ref{app:additional_results}.
...
\subsection{CIFAR-10 Non-IID Robustness}

Table~\ref{tab:cifar10_noniid_appendix} reports the CIFAR-10 counterpart of the Non-IID robustness experiment under linear heterogeneous noise.
Compared with the CIFAR-100 results in the main text, CIFAR-10 shows a milder but still visible dependence on the partitioning strength.
FedRAA achieves the strongest accuracy at $\alpha=0.1$ and $\alpha=1.0$, while FedGreed is strongest at $\alpha=0.5$.
This result complements the main-table conclusion: the benefit of pre-training admission is not limited to the harder CIFAR-100 setting, but its relative ranking can change with the dataset and heterogeneity level.
...
\label{tab:cifar10_noniid_appendix}
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.10}
\begin{tabular}{lccc}
\toprule

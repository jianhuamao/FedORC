...
\begin{figure*}[t]
\centering\includegraphics[width=\linewidth]{figure/display.png}
\caption{
...
Accordingly, the theory should be interpreted as a conditional justification: when the proxy residuals are small(more details in appendix \textcolor{red}{~\ref{app:entropy_residual}--\ref{app:smoothing_residual}}), minimizing the entropy--KL surrogate controls the target-risk bound.
...
As shown in Fig.~\ref{fig:theoretical_bound}, the FedRAA ideal subset achieves the lowest theoretical risk bound, since it simultaneously reduces the label-noise term and the distribution-mismatch term. 
...
As shown in Fig.~\ref{fig:risk_bound}, the FedRAA ideal subset achieves both the lowest target risk and the smallest gap between the empirical risk and the theoretical bound. 
...
Moreover, by comparing the clean-only skewed strategy with the balanced noisy strategy in Fig.~\ref{fig:theoretical_bound} and Fig.~\ref{fig:risk_bound}, we observe that label noise can be more harmful than label-prior skewness in this controlled setting. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figure/oracle/theoretical_bound.pdf}
    \caption{Theoretical risk bound under different oracle admission strategies. 
    The FedRAA ideal subset obtains the lowest bound by jointly reducing label-noise risk and distribution mismatch.}
    \label{fig:theoretical_bound}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figure/oracle/risk_bound.png}
    \caption{Empirical target risk and theoretical upper bound under four admission strategies. 
    $\mathcal{E}_T(h_t)$ denotes the target risk at round $t$, $B_1(S,h_t)$ denotes the theoretical upper bound, and the gap is defined as 
    $B_1(S,h_t)-\mathcal{E}_T(h_t)$.}
    \label{fig:risk_bound}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figure/oracle/acc.pdf}
    \caption{Test accuracy curves of different oracle admission strategies.  }
    \label{fig:oracle_acc}
\end{figure}
...
FedRAA remains competitive among the compared noisy-label FL baselines, obtaining the best result on CIFAR-10 under Gaussian noise and on both CIFAR-100 noise settings.
On CIFAR-100, FedRAA substantially improves over All, FedCS, FedFixer, and FedDiv.
The MedMNIST columns make the dataset-dependent nature of final accuracy explicit: FedGreed is strongest under linear noise, while LASA is strongest under Gaussian noise.
...
\begin{table*}[t]
\centering
\caption{Test accuracy (\%) of representative noisy-label FL methods under heterogeneous label noise. Results are collected at $\alpha=0.5$ with 50 clients and reported as mean$\pm$standard deviation. Best results are highlighted in bold and second-best results are underlined.}
...
FedGreed~\cite{kritharakis2025fedgreed} & \textbf{83.09$\pm$3.17} & 82.22$\pm$3.67 & \textbf{56.30$\pm$1.78} & 54.24$\pm$1.68 & 20.79$\pm$1.05 & 32.18$\pm$0.86 \\
LASA~\cite{xu2025lasa} & 54.97$\pm$0.51 & \textbf{87.52$\pm$0.57} & 42.97$\pm$0.23 & \underline{56.20$\pm$0.31} & 23.69$\pm$0.15 & 31.03$\pm$0.12 \\
FedNed~\cite{lu2024fedned} & 64.29$\pm$0.54 & \underline{86.52$\pm$0.58} & 39.72$\pm$0.09 & 55.41$\pm$0.30 & 23.63$\pm$0.04 & \underline{32.73$\pm$0.11} \\
\midrule
...
\midrule
FedRAA (ours) & \underline{81.05$\pm$0.80} & 85.71$\pm$0.37 & 53.38$\pm$1.20 & \textbf{58.68$\pm$1.81} & \textbf{28.73$\pm$0.75} & \textbf{33.17$\pm$0.84} \\
\bottomrule
\end{tabular}
\end{table*}

Table~\ref{tab:efficiency_linear} further summarizes the cost saving obtained by FedRAA at matched target accuracy.
For each scenario, the reference baseline is the strongest non-FedRAA method in Table~\ref{tab:noisy_fl_accuracy} that reaches the target.
...
\begin{table*}[!t]
\centering
\caption{FedRAA cost saving at matched target accuracy. For each scenario, the reference method is the strongest non-FedRAA method in Table~\ref{tab:noisy_fl_accuracy} that reaches the target. Computation is reported in $10^6$ FEP and communication in GB. Saving is reported as computation / communication reduction relative to the reference method.}
\label{tab:efficiency_linear}
\setlength{\tabcolsep}{4.5pt}
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{llccccc}
\toprule
\textbf{Scenario} & \textbf{Method} & \textbf{Target} & \textbf{Round} & \textbf{Comp.} & \textbf{Comm.} & \textbf{Saving} \\
\midrule
\multirow{2}{*}{CIFAR-10 Linear} & FedGreed~\cite{kritharakis2025fedgreed} & \multirow{2}{*}{50\%} & 8 & 8.49 & 33.33 & -- \\
& FedRAA (ours) & & 21 & \textbf{3.08} & \textbf{11.17} & \textbf{63.7\% / 66.5\%} \\
\midrule
\multirow{2}{*}{CIFAR-10 Gaussian} & LASA~\cite{xu2025lasa} & \multirow{2}{*}{55\%} & 20 & 21.23 & 83.32 & -- \\
& FedRAA (ours) & & 17 & \textbf{4.47} & \textbf{18.33} & \textbf{79.0\% / 78.0\%} \\
\midrule
\multirow{2}{*}{MedMNIST Linear} & FedGreed~\cite{kritharakis2025fedgreed} & \multirow{2}{*}{80\%} & 5 & 1.17 & 20.83 & -- \\
& FedRAA (ours) & & 7 & \textbf{1.04} & \textbf{14.66} & \textbf{11.1\% / 29.6\%} \\
\midrule
\multirow{2}{*}{MedMNIST Gaussian} & LASA~\cite{xu2025lasa} & \multirow{2}{*}{85\%} & 16 & 3.75 & 66.65 & -- \\
& FedRAA (ours) & & 8 & \textbf{1.78} & \textbf{26.16} & \textbf{52.5\% / 60.8\%} \\
\bottomrule
\end{tabular}
\end{table*}
...
FedRAA consistently improves over full participation by $3.28$--$4.62$ percentage points and remains competitive across all three heterogeneity levels.
...
FedGreed~\cite{kritharakis2025fedgreed} & 8.73$\pm$1.45 & 20.79$\pm$1.05 & \textbf{29.03$\pm$0.85} \\
...
FedCorr~\cite{xu2022fedcorr} & \textbf{22.03$\pm$1.01} & \underline{26.19$\pm$0.16} & 24.03$\pm$0.24 \\
...
\midrule
FedRAA (ours) & \underline{21.67$\pm$0.27} & \textbf{28.73$\pm$0.72} & \underline{28.93$\pm$0.52} \\
\bottomrule
\end{tabular}
\end{table}
...
\label{para:comp_cs}
We compare FedRAA with multiple baselines under different label-noise levels to validate the effectiveness of the proposed method. For a fair comparison, all methods are evaluated under the same communication budget and client participation setting, where 20 out of 50 clients are selected to participate in federated training.
...
\begin{table*}[!t]
\centering
\caption{Test accuracy (\%) of client selection methods under three heterogeneous noise settings. Results are reported as mean$\pm$standard deviation over the converged stage. Best results are highlighted in bold, and second-best results are underlined.}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figure/hyperparameter_sensitivity.png}
...
Random selection     & -- & -- & -- & 39.81 \\
All clients          & -- & -- & -- & 39.67 \\
Reliability only     & \checkmark & $\times$ & $\times$ & \underline{55.60} \\
Represent. only      & $\times$ & \checkmark & $\times$ & 30.69 \\
w/o adaptive calib.  & \checkmark & \checkmark & $\times$ & 53.22 \\
\midrule
FedRAA               & \checkmark & \checkmark & \checkmark & \textbf{56.60} \\
...
\end{proof}
\subsection{proof corollary\ref{cor:proxy_consistency_surrogate}}
\begin{proof}
...
\begin{figure}[t]
    \includegraphics[width=\linewidth]{figure/数据分布/线性加噪/cifar100.png}
    \caption{Class-wise clean/noisy sample composition of selected client subsets on CIFAR-100 under linear heterogeneous noise.}
    \label{fig:subset_composition_cifar100}
\end{figure}
\begin{figure}[t]
    \includegraphics[width=\linewidth]{figure/数据分布/线性加噪/tinyimagenet.png}
    \caption{Class-wise clean/noisy sample composition of selected client subsets on TinyImageNet under linear heterogeneous noise.}
    \label{fig:subset_composition_tiny}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.32\linewidth]{figure/screening.png}
    \hfill
    \includegraphics[width=0.32\linewidth]{figure/nips/scale/cifar100.png}
    \hfill
    \includegraphics[width=0.32\linewidth]{figure/nips/scale/tinyimagenet.png}
...
The results show that the relative ranking is dataset dependent: FedRAA achieves the strongest accuracy at strong Non-IID, while FedGreed is strongest at moderate and weak Non-IID levels.
...
FedGreed~\cite{kritharakis2025fedgreed} & 21.18$\pm$0.00 & \textbf{83.09$\pm$3.17} & \textbf{89.33$\pm$2.27} \\
...
\midrule
FedRAA (ours) & \textbf{47.71$\pm$1.08} & \underline{81.05$\pm$0.80} & \underline{86.82$\pm$0.10} \\
\bottomrule
\end{tabular}
\end{table}

...
As shown in Fig.~\ref{fig:oracle_bound}, the FedRAA ideal subset achieves the lowest theoretical risk bound, since it simultaneously reduces the label-noise term and the distribution-mismatch term.
...
As shown in Fig.~\ref{fig:oracle_risk}, the FedRAA ideal subset achieves both the lowest target risk and the smallest gap between the empirical risk and the theoretical bound.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.62\textwidth]{figure/oracle/risk_bound.png}
...
\begin{figure}[t]
    ...
    \caption{Test accuracy curves under the same oracle admission strategies.}
...
FedRAA improves over full participation by $4.05$--$14.22$ percentage points and achieves the strongest accuracy at $\alpha=0.1$ and $\alpha=1.0$, while FedGreed is strongest at $\alpha=0.5$.
The corresponding CIFAR-100 and MedMNIST results are provided in Appendix~\ref{app:additional_results}.
...
FedGreed~\cite{kritharakis2025fedgreed} & 23.55$\pm$1.98 & \textbf{56.30$\pm$1.78} & 55.75$\pm$1.52 \\
...
FedCorr~\cite{xu2022fedcorr} & 25.29$\pm$1.43 & \underline{53.39$\pm$0.33} & 44.93$\pm$0.50 \\
...
\midrule
FedRAA (ours) & \textbf{30.27$\pm$0.75} & 53.38$\pm$1.20 & \textbf{57.48$\pm$1.07} \\
\bottomrule
\end{tabular}
\end{table}
...
This result complements the main-table conclusion: the benefit of pre-training admission is visible beyond CIFAR-10, but the relative ranking can change with the dataset difficulty and heterogeneity level.

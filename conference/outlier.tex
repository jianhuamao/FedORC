...
In this section, we first use a controlled oracle experiment to sanity-check the theoretical bound. We then ask whether FedOCR removes harmful client blocks early enough to improve robustness, reduce system cost, and reshape the retained pool in a way that matches the intuition behind the method.
...
The table shows that the main benefit of FedOCR is not a small refinement on top of existing methods, but an early removal of the client blocks that most distort optimization.
Compared with full participation, standalone FedOCR improves all six dataset-noise settings, and the gain is largest on the linear regimes where client-level corruption is easiest to isolate before it spreads through training.
That pattern is important: it says the admission stage is most valuable when the dominant failure mode is harmful participation itself rather than a residual sample-level error that only appears after a pool has already been selected.
The combination rows add a second lesson. FedOCR is modular, but not uniformly additive: its value depends on whether the downstream method addresses the remaining corruption pattern.
FedOCR+FedNoRo is strongest in the four columns where a robust full-round learner can best exploit the cleaner pool produced by admission, while FedOCR+FedCorr and FedOCR+FedCor win in the two linear cases where the downstream correction is better matched to the remaining corruption structure.
So the table supports a cleaner conclusion than a raw leaderboard would suggest: admission gives the core robustness gain, and downstream correction is a regime-dependent refinement rather than the primary source of improvement.
...
The table makes a sharper point than a single accuracy comparison: admission is the place where most wasted work is eliminated.
...
When a downstream correction method is attached, the result is no longer purely about cost reduction; it becomes a trade-off between extra overhead and better recovery of the residual noise.
That is why the combined variants sometimes improve the target cost and sometimes do not: the downstream stage helps only when its correction ability is strong enough to justify the additional rounds.
So the table supports a system view of FedOCR, not just an accuracy view: the pre-training filter is the main lever for cutting cost, while downstream correction is a conditional refinement.
...
The interesting trend here is not just that FedOCR stays competitive, but that its advantage is largest when heterogeneity becomes most hostile.
As the partition becomes less extreme, all methods improve, but FedOCR remains near the top and wins at the two ends of the range, which suggests that its client-block filtering is especially useful when the learning problem is dominated by skew and corruption rather than by mild heterogeneity alone.
FedGreed leading at the middle point is also informative: when the data are heterogeneous but not yet severely corrupted, utility-based selection can be enough to exploit diversity.
FedOCR, in contrast, is designed for the harder regime where reliability filtering matters more than diversity alone.
So the sweep says that FedOCR is not a narrow fix for one partition strength; it is most valuable when the partition is both noisy and skewed enough that cleaning the pool matters as much as choosing it.
\begin{table*}[!t]
	\centering
...
The sweeps are most useful as a diagnosis of the admission mechanism itself.
They show that FedOCR is governed by a real trade-off rather than a one-way preference for stronger filtering: too little pressure leaves noisy clients in the pool, while too much pressure prunes away useful coverage and hurts the downstream learner.
The best region sits in the middle, which is exactly what we want from a practical admission rule: it should be selective enough to remove risk, but not so aggressive that it turns the retained set into a tiny or biased sample.
So the figure is not just tuning advice; it shows that FedOCR behaves like a balanced selector instead of a brittle heuristic.
...
The size sweep shows that the probing set behaves like a signal budget rather than a training set: if it is too small, the entropy and coverage estimates become noisy; if it is too large, the admission stage spends more scans without improving the ranking.
The Dirichlet sweep tells a complementary story: what matters is not that the probe be perfectly IID, but that it exposes enough class support for the KL-based coverage signal to remain discriminative.
The label-noise sweep is flat by design, because FedOCR never consumes the probing labels at all.
That invariance is important: it confirms that the probe contributes through its inputs and class coverage only, so label corruption on the unused annotations cannot perturb the retained client set.
Taken together, the table says that FedOCR is sensitive to the informative part of the probing set, but intentionally blind to the labels it does not use.
\begin{table}[!t]
	\centering
...
The figure shows that FedOCR is not just pruning more aggressively; it is reshaping the retained pool.
Compared with the alternatives, it keeps a client subset that is simultaneously cleaner and more class-complete, which matters because either property alone is not enough for stable federated optimization.
Methods that chase reliability alone can leave coverage gaps, while methods that chase diversity alone can preserve noisy clients.
FedOCR avoids both failure modes by balancing the two, so the retained pool is better prepared for downstream training before any learning step begins.
...
Reliability filtering is the main driver, because once high-risk clients are removed the retained pool is much easier to optimize even without any representativeness term.
Representativeness alone cannot rescue a noisy pool, which is why the distribution-only variant still performs poorly.
The adaptive calibration then adds the final refinement by preventing the method from committing to a fixed reliability-versus-coverage balance across all regimes.
So the ablation does not just show that every component helps; it shows that the components play different roles, with reliability as the gatekeeper, representativeness as the coverage safeguard, and calibration as the regime adapter.

...
The oracle experiment is summarized briefly here; Appendix~\ref{app:oracle_experiment_details} provides the full setup and bound plots.
Figure~\ref{fig:oracle_accuracy} shows that the oracle FedOCR-style retention strategy achieves the best accuracy among the four retention rules, consistent with the theoretical ranking.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.92\linewidth]{figure/oracle/acc.pdf}
    \caption{Test accuracy curves under the oracle retention strategies.}
...
\label{subsec:exp_setup}
	...
	\begin{itemize}
		...
		\setlength{\topsep}{0.15em}
		\setlength{\parsep}{0pt}
		\item \textbf{Noisy-label FL methods:} RFA~\cite{pillutla2022robust}, FedGreed~\cite{kritharakis2025fedgreed}, LASA~\cite{xu2025lasa}, FedNed~\cite{lu2024fedned}, FedNoRo~\cite{wu2023fednoro}, FedCorr~\cite{xu2022fedcorr}, FedCS~\cite{hao2025fedcs}, FedFixer~\cite{ji2024fedfixer}, and FedDiv~\cite{li2024feddiv}.
		\item \textbf{Client-selection methods:} All, RS, EMD, GS~\cite{ma2021client}, PoC~\cite{cho2022power}, and FedCor~\cite{tang2022fedcor}.
	\end{itemize}

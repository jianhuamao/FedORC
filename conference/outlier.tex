...
Our goal is to construct, using only client-level signals, a retained client-block set that approximates the ideal sample-level outlier-removal set (ORS) from centralized outlier-removal literature~\cite{natarajan2013noisy,patrini2017loss,han2018coteaching,northcutt2021confident,zhang2021nochange}.
In FL, however, privacy and locality constraints prevent sample-wise intervention~\cite{kairouz2021advances}; the feasible decision unit is an entire client block, which gives rise to the block-constrained client-level outlier-removal set (BC-ORS).
BC-ORS is therefore the practical FL objective: it is coarser than ORS and may be slightly suboptimal in accuracy, but it can be identified from client-level signals and admits a much more efficient search.
In this section, we provide a theoretical guarantee that BC-ORS-style pre-training filtering is aligned with reducing the target-risk upper bound.
...
Because FL cannot realize sample-level ORS, the relevant question is whether a retained client-block set $S$ lowers the target-risk upper bound.
Changing $S$ changes the training distribution induced by the selected local data blocks~\cite{hsu2019measuring,li2021silos,zhu2021survey}: some retained clients may inject noisy supervision, while others may shift the retained pool away from the target distribution.
Our analysis shows that both effects increase the target-risk upper bound.
This provides the theoretical guarantee behind FedOCR: it aims to remove unreliable clients while preserving enough representative ones, so that the retained pool is a practical BC-ORS approximation rather than an unattainable ORS.

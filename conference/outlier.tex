...
The table itself makes the effect of admission easy to read: compared with full participation, standalone FedOCR improves all six dataset-noise settings, with the largest gains on MedMNIST linear noise ($60.89\%\!\rightarrow\!81.05\%$) and CIFAR-10 linear noise ($39.16\%\!\rightarrow\!56.60\%$), where harmful client blocks are most damaging.
This means that the admission stage is already a strong denoising front-end before any downstream correction is applied.
The four FedOCR rows also show its modularity more concretely: FedOCR+FedNoRo is best in four columns, namely MedMNIST linear ($\boldsymbol{86.19\pm0.41}$), MedMNIST Gaussian ($\boldsymbol{91.32\pm0.39}$), CIFAR-10 Gaussian ($\boldsymbol{67.89\pm0.35}$), and CIFAR-100 Gaussian ($\boldsymbol{35.49\pm0.13}$); FedOCR+FedCorr is best on CIFAR-10 linear ($\boldsymbol{57.01\pm0.84}$); and FedOCR+FedCor is best on CIFAR-100 linear ($\boldsymbol{30.94\pm0.23}$).
Overall, Table~\ref{tab:noisy_fl_accuracy} shows that FedOCR is not only better than full participation on its own, but also a useful admission front-end that can be paired with different downstream FL methods to obtain the strongest result in a given noise regime.

...
As shown in table\ref{tab:noniid_robustness}, we also test whether the conclusion changes with the strength of Non-IID partitioning.
...
For the first-round epoch budget, 10 epochs give the most useful filtering indicators: too few epochs under-adapt the client models and yield noisy entropy estimates, whereas too many epochs make even risky clients overconfident on the probing set, shrinking the entropy gap and making the admission rule overly permissive.
...
This table primarily tests how demanding the probing set itself is.
...
The size sweep is the key result: even a 1\% probing set, which corresponds to only 100 images or about 10 images per class, keeps accuracy in a very tight band and only slightly below the best 5\% setting.
This shows that FedOCR does not need a large clean probe to work well, so the data-collection cost of the probing set is low.
The Dirichlet sweep shows a smaller but still visible effect: mild changes in the probing input distribution slightly perturb accuracy and the retained count, which means the probe does not need to be perfectly IID, only sufficiently class-covering for the KL signal to stay useful.
The label-noise sweep is exactly flat because FedOCR never consumes the probing labels at all.
This is the strongest practical takeaway: probe-label quality is not a real requirement for FedOCR, so label corruption on the unused annotations does not change the selected clients, the training trajectory, or the final accuracy.
Taken together, the table says that FedOCR is robust to probe size, tolerant to mild probe distribution shift, and invariant to probing-set label noise, which greatly lowers the practical difficulty of building the probing set.
\begin{table}[!t]
	\centering

...
This section provides a theoretical guarantee for that block-level design: we show that BC-ORS-style pre-training filtering is aligned with reducing the target-risk upper bound.
...
Our analysis shows that two factors matter: unreliable clients can inject noisy supervision, and an imbalanced retained pool can drift away from the target distribution~\cite{hsu2019measuring,li2021silos,zhu2021survey}.
Both effects increase the target-risk upper bound, which yields the theoretical guarantee behind FedOCR: it seeks to remove unreliable clients while preserving enough representative ones, so the retained pool is a practical BC-ORS approximation rather than an unattainable ORS.

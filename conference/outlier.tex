...
\author{\IEEEauthorblockN{Jianhua Mao\IEEEauthorrefmark{2}, Junqing Yin\IEEEauthorrefmark{2}, Wei Xi\IEEEauthorrefmark{2}\textsuperscript{$\dag$}, Hanlin Gu\IEEEauthorrefmark{4}, Lixin Fan\IEEEauthorrefmark{4}\textsuperscript{*}}
...
\thanks{\textsuperscript{*} Corresponding author: Lixin Fan (linxi.fan01@gmail.com). \textsuperscript{$\dag$} Co-corresponding author: Wei Xi (xiwei@xjtu.edu.cn).}
}
...
In this section, we evaluate FedOCR on three benchmark datasets under heterogeneous label-noise and Non-IID settings, comparing it with representative noisy-label FL and client-selection baselines, and further studying its efficiency, sensitivity, and ablation behavior.	
...
Unless otherwise specified, the training data are distributed over $N=50$ clients. 
...
\begin{itemize}
	...
	\item \textbf{Client-selection baselines:} GS~\cite{ma2021client}, PoC~\cite{cho2022power}, and FedCor~\cite{tang2022fedcor}.
\end{itemize}
...
For the strength coefficient, the best result is obtained at strength $=1$; this coefficient controls how strongly the selector shifts the greedy utility between distribution coverage and reliability according to the pool-level entropy profile. A smaller value keeps the balance closer to the default setting and gives noisy clients too much influence, while a larger value overreacts to the entropy signal and can tilt the greedy ranking away from useful clients.
For the first-round epoch budget, 10 epochs give the clearest filtering signal: shorter runs underfit the entropy estimate, while longer runs let even risky clients adapt too much to the probing set and blur the gap between reliable and risky clients.
For $\eta$, the default range $[0.10,0.30]$ selects 20 clients and achieves the best accuracy; a narrower range is too permissive and lets noisy clients slip through, while a wider range is too restrictive and drops useful clients.
...
RS keeps a broader pool, but it also keeps more noisy mass. Fixed is the oracle comparator obtained with access to the true target distribution, so it serves as a theoretical upper-bound reference rather than a deployable selection rule.
...
\textbf{Privacy boundary.} FedOCR is privacy-compatible, but it is not a formal privacy-preserving protocol. Although the probing set itself does not expose client data, server-client interaction may still leak coarse distributional information. A malicious server could, in principle, craft probing samples to infer whether a client contains certain classes or patterns. Stronger privacy defenses, such as differentially private entropy estimation~\cite{suresh2019dpanonymizedhistograms,to2019dplocationentropy}, probe sanitization via privacy-preserving data cleaning~\cite{krishnan2016privateclean}, and encrypted or secure evaluation via similarity and homomorphic-encryption protocols~\cite{blundo2014espresso,shen2024mkfhe,zhao2023treeinference}, are left for future work.

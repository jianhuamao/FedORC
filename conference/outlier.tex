...
To provide a controlled validation of our theoretical analysis, we design a small-scale oracle experiment in which each client's clean label prior and label-noise rate are explicitly specified. 
This experiment is not meant to benchmark practical methods; its role is to test whether the risk factors in Theorem~\ref{thm:main_bound} rank retained client-block sets in the expected order.

\paragraph{Experimental Setup}
We consider a three-class Gaussian classification problem with two-dimensional input features. 
Each class shares the same covariance but has a different mean. 
The target distribution is class-balanced, while the six candidate clients differ in both label prior and symmetric label-noise rate. 
For each client, the true label is flipped uniformly to one of the other $K-1$ classes with probability $\rho$. 
...
\end{itemize}
The first strategy corresponds to no client-level outlier removal. 
The next two strategies isolate the effects of distribution mismatch and label noise, respectively. 
The last strategy is the oracle version of the reliability- and representativeness-aware admission rule advocated by FedOCR.

\paragraph{Results}
We first compute the theoretical upper bound from Theorem~\ref{thm:main_bound} for each retained subset. 
As shown in Fig.~\ref{fig:oracle_bound}, the FedOCR ideal subset achieves the lowest bound because it reduces both the label-noise term and the distribution-mismatch term. 
By contrast, the balanced noisy subset has the largest bound, showing that coverage alone cannot compensate for severe corruption.

We then compare the empirical target risk with the bound during training. 
As shown in Fig.~\ref{fig:oracle_risk}, the FedOCR ideal subset achieves the lowest target risk and the smallest bound gap. 
This supports the main theoretical message: low noise and good coverage are both needed, and neither is sufficient on its own. 
The comparison between the clean-only skewed and balanced noisy subsets further suggests that, in this toy setting, label noise is more damaging than mild prior skewness, which motivates FedOCR's adaptive balancing rule.

...
\begin{figure}
    ...
    \caption{\small Final test accuracy with different numbers of retained clients under noisy and Non-IID settings. 
...
FedOCR then uses an adaptive greedy rule to integrate reliability and representativeness signals and construct the retained client set. Fig.~\ref{fig:top50-intro} illustrates why the retained set size should not be fixed blindly: retaining too few clients may lose target coverage, whereas retaining too many can reintroduce accumulated noise. Accordingly, FedOCR aims to retain a client set that contains sufficient clean data while preserving balanced class coverage. As shown in Fig.~\ref{fig:display}(b) and Fig.~\ref{fig:top50-intro}, such a low-noise and complementary client pool yields the best empirical performance.
...
\paragraph{Centralized outlier removal}
...
\paragraph{Noisy-label learning in FL}
...
\paragraph{Client-level dynamic outlier removal}
...
We study \emph{one-shot client-block outlier removal} before federated optimization starts.
To make the analysis comparable to centralized outlier-removal literature, we first recall the ideal sample-level outlier-removal set (ORS): if the server could inspect all samples directly, it would surgically remove individual corrupted examples and retain the best sample subset for the target risk~\cite{natarajan2013noisy,patrini2017loss,han2018coteaching,northcutt2021confident,zhang2021nochange}.
In FL, however, privacy and locality constraints prevent such sample-wise intervention~\cite{kairouz2021advances}; the feasible decision unit is an entire client block, which gives rise to the block-constrained client-level outlier-removal set (BC-ORS).
BC-ORS is a coarser surrogate of ORS: it may be slightly suboptimal in accuracy because it cannot delete only the corrupted samples inside a client block, but it is the practical objective in FL because it can be identified using client-level signals rather than sample-level auditing.
As illustrated in Fig.~\ref{fig:display}(c) and the experiments in Section~\ref{sec:experiment}, this block-level search is orders of magnitude more efficient and therefore more deployable in FL.
...
Consider the centralized setting where all training samples are directly accessible.
Let $D=\bigsqcup_{i=1}^{N}D_i$ denote the full training sample collection underlying the candidate pool, and let $h_{\mathcal G}$ denote the model trained on an arbitrary retained sample subset $\mathcal G\subseteq D$.
The sample-level optimal retained set is
\begin{equation}
\label{eq:sample_level_optimum}
\mathcal G^\star_{\mathrm{ORS}}
\in
\arg\min_{\emptyset\neq \mathcal G\subseteq D}
\epsilon_T(h_{\mathcal G}).
\end{equation}
We call $\mathcal G^\star_{\mathrm{ORS}}$ the sample-level outlier-removal set (ORS).
\end{definition}
% \begin{assumption}[Full participation after outlier removal]
% Given a candidate client pool $\mathcal{C}$, a retained subset 
...
Clients in $\mathcal{C}_{\mathrm{risk}}$ have persistent risks, such as severe label noise or distributional bias, which can repeatedly affect global aggregation once they are retained for training.
\end{assumption}

\begin{definition}[Block-constrained client-level outlier-removal set (BC-ORS) in FL]
...
From our perspective, the server cannot freely select arbitrary samples from $D$ as in centralized noisy-label learning.
Instead, privacy and locality constraints restrict feasible pre-training control to unions of client blocks~\cite{mcmahan2017communication,kairouz2021advances}:
\[
...
S^\star_{\mathrm{BC-ORS}}
\in
\arg\min_{\emptyset\neq S\subseteq \mathcal{C}}
\epsilon_T(h_S).
\end{equation}
Here, $S^\star_{\mathrm{BC-ORS}}$ denotes the block-constrained client-level outlier-removal set (BC-ORS). 
Unlike ORS, BC-ORS cannot excise only the noisy samples inside a client block, so it is generally a slightly coarser and accuracy-relaxed surrogate of the centralized sample-level optimum.
Ideally, the candidate pool can be decomposed into the retained outlier-removal client set and the removed risk-prone clients, i.e.,
\[
\mathcal{C}=S^\star_{\mathrm{BC-ORS}}\cup\mathcal{C}_{\mathrm{risk}}.
\]
Our goal is to approximate the BC-ORS optimum in Eq.~\eqref{eq:block_constrained_optimum} through a computable pre-training client-level outlier-removal rule.
\end{definition}

\begin{remark}[ORS versus BC-ORS]
\label{rem:ORS_vs_BC-ORS}
ORS is the sample-level oracle benchmark, because it can remove individual corrupted examples when raw data are directly accessible.
BC-ORS is the FL-constrained counterpart, because it can only keep or discard whole client blocks and is therefore generally slightly suboptimal to ORS in target accuracy.
We nevertheless prefer BC-ORS in FL because privacy and locality constraints rule out sample-level auditing, and because finding BC-ORS is orders of magnitude more efficient in communication and computation, as illustrated in Fig.~\ref{fig:display}(c) and Section~\ref{sec:experiment}.
\end{remark}

\subsection{A Unified Risk View of Client-Block Outlier Removal}
\label{subsec:risk_view}

We justify pre-training client-level outlier removal from a risk perspective.
ORS is the centralized oracle benchmark, but FL cannot realize it because the server cannot directly remove individual noisy samples from private client data~\cite{mcmahan2017communication,kairouz2021advances}.
Therefore, the practical unit of filtering is the client itself, which makes BC-ORS the relevant deployable objective.
If we keep a different client set $S$, we train on a different collection of local data blocks, and thus on a different training distribution~\cite{hsu2019measuring,li2021silos,zhu2021survey}.
Some retained clients may bring many noisy labels, while others may make the retained data distribution deviate from the target distribution.
Our analysis shows that both problems increase the target-risk upper bound.
This motivates FedOCR to remove unreliable clients while keeping enough representative clients before training starts, thereby searching for a practical approximation to BC-ORS rather than the unattainable ORS.
...
The bound in Theorem~\ref{thm:main_bound} lives at the population level, whereas FedOCR must operate with information that is observable before training.
...
These quantities cannot be inspected by the server under the privacy and locality constraints of FL~\cite{kairouz2021advances}.
FedOCR therefore instantiates the two risk factors through probing-set probing statistics.
This step is not treated as an exact substitution.
...
Theorem~\ref{thm:residualized_probing_bound} does not assume that predictive entropy or probing-predicted signatures are universally accurate.
Instead, it separates the observable part of the bound from the residual terms that quantify probing mismatch.
...
It introduces a one-time pre-screening cost, but avoids repeatedly auditing or repairing unreliable client blocks during later communication rounds, a cost-sensitive motivation shared with efficient client selection and noisy-client pruning~\cite{lai2021oort,fu2023client,morafah2024clipfl}.
...
If the budget is too large, the server may be forced to retain unreliable client blocks; if it is too small, useful and target-complementary clients may be discarded.
We therefore allow the final participant pool size to be determined by the outlier-removal sequence, and introduce a temperature coefficient to explicitly control the retained cardinality.
...
•
We evaluate FedOCR from five complementary perspectives:
\begin{itemize}
\setlength{\itemsep}{0.15em}
\setlength{\topsep}{0.2em}
\setlength{\parsep}{0pt}
	\item \textbf{Oracle validation.} We check whether the risk factors
	in Theorem 1 explain the quality of different retained
	client-block sets.
	\item \textbf{Main comparison.} We compare FedOCR with noisy label FL methods and client selection baselines under
	coupled label noise and Non-IID heterogeneity.
	\item \textbf{Sensitivity analysis.}  We study how Non-IID strength,
	admission hyperparameters, and probing-set construction affect the retained client pool and final
	accuracy.
	\item \textbf{Subset analysis.} We analyze the outlier-removal client
	sets to verify whether the retained clients are both
	reliable and representative.
	\item \textbf{Ablation study. }  We isolate the effects of reliability
	filtering, distributional coverage control, and the
	noise-adaptive calibration mechanism.
\end{itemize}
...
This experiment is not intended to compare FedOCR with practical baselines. 
Instead, it provides a controlled validation of whether the risk factors in Theorem~\ref{thm:main_bound} can explain the quality of different retained client-block sets.
...
We compare four representative retention strategies:
\begin{itemize}
    \item \textbf{All clients}: all candidate clients are retained without outlier removal.
    \item \textbf{Clean-only skewed}: reliability outliers are removed, but the retained low-noise clients are class-skewed.
    \item \textbf{Balanced noisy}: distributional coverage is preserved, but noisy client blocks are still retained.
    \item \textbf{FedOCR ideal}: clients that are both low-noise and distributionally complementary are retained.
\end{itemize}
The first strategy corresponds to no client-level outlier removal, while the next two strategies isolate the effects of distribution mismatch and label noise, respectively. 
The last strategy represents the oracle version of the reliability- and representativeness-aware outlier-removal principle advocated by FedOCR.

\paragraph{Results}
We first compute the theoretical upper bound derived in Theorem~\ref{thm:main_bound} for different retained subsets. 
As shown in Fig.~\ref{fig:oracle_bound}, the FedOCR ideal subset achieves the lowest theoretical risk bound, since it simultaneously reduces the label-noise term and the distribution-mismatch term.
In contrast, the balanced noisy strategy leads to the largest bound, indicating that distributional balance alone is insufficient when the retained clients contain severe label noise.

We further compare the empirical target risk with the corresponding theoretical upper bound during training. 
As shown in Fig.~\ref{fig:oracle_risk}, the FedOCR ideal subset achieves both the lowest target risk and the smallest gap between the empirical risk and the theoretical bound.
...
Moreover, by comparing the clean-only skewed strategy with the balanced noisy strategy in Figs.~\ref{fig:oracle_risk} and~\ref{fig:oracle_accuracy}, we observe that label noise can be more harmful than label-prior skewness in this controlled setting.
This observation further motivates the dynamic weighting mechanism in FedOCR, 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.92\linewidth]{figure/oracle/risk_bound.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.92\linewidth]{figure/oracle/acc.pdf}
    \caption{Test accuracy curves under the same oracle retention strategies.}
...
\setlength{\topsep}{0.2em}
\setlength{\parsep}{0pt}
	\item We evaluate FedOCR through five complementary questions:
	\begin{itemize}
	\setlength{\itemsep}{0.1em}
	\setlength{\topsep}{0.15em}
	\setlength{\parsep}{0pt}
		\item \textbf{Does client-block outlier removal improve robustness in harsh noisy and Non-IID settings?} We compare FedOCR with noisy-label FL methods and representative client-selection baselines.
		\item \textbf{Can the standalone admission rule reach matched target utility with lower cost?} We analyze whether the pre-training client-block filter reaches the target with lower computation and communication cost.
		\item \textbf{How does the retained pool change as heterogeneity varies?} We examine how the selected clients evolve as the data heterogeneity level changes.
		\item \textbf{How sensitive is the admission stage to its own design choices?} We study the effect of hyperparameters and probing-set construction.
		\item \textbf{Why does the method work?} We inspect the composition of the retained subsets and ablate the scoring terms.
	\end{itemize}
	\item We compare against two baseline groups:
	\begin{itemize}
	\setlength{\itemsep}{0.1em}
	\setlength{\topsep}{0.15em}
	\setlength{\parsep}{0pt}
		\item \textbf{Noisy-label FL baselines.} Full-round robust training baselines RFA~\cite{pillutla2022robust}, FedGreed~\cite{kritharakis2025fedgreed}, LASA~\cite{xu2025lasa}, FedNed~\cite{lu2024fedned}, and FedNoRo~\cite{wu2023fednoro}, together with training-stage correction methods FedCorr~\cite{xu2022fedcorr}, FedCS~\cite{hao2025fedcs}, FedFixer~\cite{ji2024fedfixer}, and FedDiv~\cite{li2024feddiv}.
		\item \textbf{Client-selection baselines.} Full participation (All), random selection (RS), distribution-aware selection (EMD and GS~\cite{ma2021client}), and optimization-driven selection (PoC~\cite{cho2022power} and FedCor~\cite{tang2022fedcor}).
	\end{itemize}
\end{itemize}
...
For FedOCR's admission stage, we use a short 10-epoch local adaptation to compute the filtering indicators, which keeps the pre-screening lightweight and consistent with the deployment-oriented design. For local training, we use Adam with learning rate $1\times10^{-4}$ and batch size 64.
...
Table~\ref{tab:noisy_fl_accuracy} reports the collected accuracy results on MedMNIST~\cite{yang2023medmnist}, CIFAR-10~\cite{krizhevsky2009learning}, and CIFAR-100~\cite{krizhevsky2009learning}.
...
For completeness, the table also includes representative client-selection baselines under the same linear and Gaussian noise settings.
The table includes four FedOCR variants: the standalone FedOCR row applies the proposed pre-training client-block outlier-removal rule and then runs standard FedAvg, while the combination rows apply the same rule before FedCor, FedCorr, or FedNoRo.
This separation lets us distinguish the direct effect of admission from its modular compatibility with downstream FL algorithms.

The standalone FedOCR row consistently improves over full participation across all six dataset-noise settings.
This shows that a coarse pre-training filter already removes a substantial portion of harmful client blocks before they repeatedly introduce corrupted supervision or target-mismatched updates.
The best combination depends on the dataset and noise type: FedOCR + FedNoRo achieves the best accuracy in four of the six settings, FedOCR + FedCorr is best on CIFAR-10 linear noise, and FedOCR + FedCor is best on CIFAR-100 linear noise.
Taken together, these results support the intended role of FedOCR as a modular admission step: it improves the quality of the participating pool and can further strengthen different downstream FL methods, but the exact gain still depends on the downstream algorithm and corruption pattern.

Table~\ref{tab:efficiency_linear} isolates the system-level picture at matched target accuracy.
For each scenario, the reference baseline is a strong non-FedOCR method in Table~\ref{tab:noisy_fl_accuracy} that reaches the target.
...
When a downstream noisy-label method is attached, the final accuracy can improve, but the extra training stage may partially offset the savings and even increase the end-to-end cost in harder settings.
...
Table~\ref{tab:noniid_robustness} reports the collected CIFAR-10 results under linear noise for $\alpha\in\{0.1,0.5,1.0\}$.
...
These results show that the proposed outlier-removal rule remains effective across both highly skewed and moderately heterogeneous partitions, but the best-performing selection strategy can still depend on the exact heterogeneity regime.
\begin{table*}[!t]
	\centering
...
\paragraph{Probing-set sensitivity}
Beyond the hyperparameters of the admission rule, the probing set itself is another important design knob because its input samples determine the observable entropy and probing-predicted coverage signals used by FedOCR.
...
All probing-set sensitivity runs use a fixed random seed, so identical rows indicate that FedOCR selected the same client set and followed the same training trajectory rather than an averaging artifact.
...
The Dirichlet sweep further shows that the input distribution of the probing set affects the admission decision by changing the probing class-coverage signal.
...
This is expected because FedOCR does not use the true labels of probing-set samples.
The probing set is used only through its inputs: client models are evaluated on these inputs to compute predictive entropy and probing-predicted class signatures.
Therefore, corrupting the unused labels of the probing set does not change the selection scores, the selected clients, or the subsequent training trajectory.
Thus, Table~\ref{tab:probing_set_sensitivity} supports a sharper conclusion: FedOCR is sensitive to probe size and input distribution, but invariant to probing-set label noise when the random seed and probe inputs are fixed.
\begin{table}[!t]
	\centering
...
FedOCR retains a subset with more clean samples and fewer noisy samples across most classes, while maintaining more balanced class coverage. 
In contrast, competing methods either accumulate more noisy samples or leave visible class gaps. 
This indicates that FedOCR improves both cleanliness and coverage at the client-block level rather than merely retaining fewer clients.
...
Using the distribution score alone performs poorly, which indicates that coverage alone cannot compensate for heavily corrupted local supervision.
The full FedOCR model further improves over the non-adaptive variant by $3.38$ percentage points, confirming that noise-adaptive calibration is needed to balance reliability and representativeness rather than relying on a fixed weighting rule.

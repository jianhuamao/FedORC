...
FedOCR uses probing-based signals because the server cannot inspect raw client data under FL privacy constraints~\cite{mcmahan2017communication,kairouz2021advances}.
That makes the method practical, but it also means the probing set matters.
Predictive entropy is only a proxy for client unreliability, not a direct estimate of the true noise rate~\cite{northcutt2021confident,guo2017calibration}.
The KL term is also only a coverage proxy relative to a server-side reference prior~\cite{ben2010theory,lipton2018labelshift}.
So FedOCR is a coarse pre-training filter by design: it removes risky client blocks before training and leaves residual sample-level noise to downstream correction if needed~\cite{xu2022fedcorr,wu2023fednoro,li2024feddiv,ji2024fedfixer}.
If the client population or target distribution changes over time, the probing set may need to be refreshed.
